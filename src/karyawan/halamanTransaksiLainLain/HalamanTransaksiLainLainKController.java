package karyawan.halamanTransaksiLainLain;
...
public class HalamanTransaksiLainLainKController implements Initializable {
    ...
    private void setKomponen(){
        cbxJenisTransaksi.getItems().addAll("Pemasukan", "Pengeluaran");
        cbxJenisTransaksi.setValue("Pemasukan");
    ...
    private void getDataTabelTransaksi(){
        ...
        String query = "SELECT id_transaksi_lain, adm.username,\n" +
        "DATE(tanggal_transaksi) AS tanggal, TIME(tanggal_transaksi) AS waktu,\n" +
        "nominal, keterangan\n" +
        "FROM transaksi_lain\n" +
        "JOIN admin adm ON adm.id_admin = transaksi_lain.id_admin\n" +
        ...
        try (PreparedStatement statement = Koneksi.getCon().prepareStatement(query)) {
            ...
            while (result.next()) {
               String idTransaksi = result.getString("id_transaksi_lain");
    ...
    private void prosesTransaksi(){
        ...
        try {
            String query = "INSERT INTO transaksi_lain VALUES (?,?,NOW(),?,?,?)";
            PreparedStatement statement = Koneksi.getCon().prepareStatement(query);
            statement.setString(1, idTransaksiLainBaru);
